...
<v-container>
    ...
    <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    ...
                    <v-card>
                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field v-model="editedItem.nombre" label="Nombre de la dependencia"></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="4">
                        ...
                        <v-card-actions>
                            ...
                            <v-btn color="blue darken-1" text @click="editarDependencia(editedItem)">
                                Modificar
                            </v-btn>
                        </v-card-actions>
                    </v-card>
        ...
        <template v-slot:item.actions="{ item }">
            ...
            <v-icon small @click="eliminarDependencia(item._id)">
                mdi-delete
            </v-icon>
        </template>
...
export default {
    data: () => ({
        ...
        dialogDelete: false,
        headers: [{
                text: 'ID  de dependencia',
                value: '_id'
            },
            {
                text: 'Nombre de dependencia',
                value: 'nombre'
            },
            {
                text: 'Usuario Id',
    ...
    methods: {
        ...
        editItem(item) {
            this.dialog = true
            console.log(item);
            this.axios.get(`buscarDependencia/${item}`)
                .then(res => {
        ...
        listarDependencias() {
            ...
            this.axios.get('/buscarDependencias')
                .then((response) => {
        ...
        editarDependencia(item) {

            this.axios.put(`/actualizarDependencia/${item._id}`, item)
                .then(res => {
                     this.$swal('success!',
        ...
        eliminarDependencia(id) {
            console.log(id);
            this.axios.delete(`/eliminarDependencia/${id}`)
                .then(res => {
                       this.$swal('OK!',
